...
// ADAPTER: Classe que adapta a interface ImportedFlower para FloresNacionais
class TulipAdapter implements FloresNacionais {
    private ImportedFlower importedFlower;
    
    public TulipAdapter(ImportedFlower importedFlower) {
        this.importedFlower = importedFlower;
    }
    
    @Override
    public void exibirDescricao() {
        System.out.println("Flor Importada: ");
        importedFlower.showDescription();
        System.out.println("País de origem: " + importedFlower.getOriginCountry());
    }
    
    @Override
    public double obterPreco() {
        // Adiciona taxa de importação de 30% ao preço original
        return importedFlower.getPrice() * 1.30;
    }
    
    @Override
    public String obterCor() {
        return importedFlower.getColor();
    }
}